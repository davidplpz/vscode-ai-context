...
export class SkillDetailProvider {
  ...
  private updateContent(): void {
    if (!this.panel || !this.currentSkill) return;
   ...
   */
  private handleMessage(message: { command: string; text?: string; path?: string }): void {
   ...
   */
  private getHtml(skill: Skill): string {
    const bodyHtml = markdownToHtml(skill.body);
    const triggersHtml = skill.triggers
      .map(
        (t) =>
          `<button class="trigger-tag" onclick="copyText('${this.escapeJs(t)}')" title="Click to copy">${this.escapeHtml(t)}</button>`
      )
      .join('');

    const metaRows = [
      { label: 'Source', value: skillSourceLabel(skill.source) },
    ...
    const metaHtml = metaRows
      .map(
        (r) =>
          `<tr><td class="meta-label">${this.escapeHtml(r.label)}</td><td class="meta-value">${this.escapeHtml(r.value)}</td></tr>`
      )
      .join('');
